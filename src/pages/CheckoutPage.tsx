...
const CheckoutPage: React.FC = () => {
    ...
    const handlePlaceOrder = async () => {
        ...
        // Lấy Token từ localStorage (vì Backend của bạn có AuthModule )
        const token = localStorage.getItem('accessToken');

        const orderPayload = {
            customer_id: 1,           // Bạn có thể lấy từ thông tin user đăng nhập
            staff_id: 2,              // ID nhân viên phục vụ bàn này
            table_id: Number(table),  // Ép kiểu về Number để khớp với BigInt/Int 
            total_amount: subtotal,
            discount_amount: 0,
            final_amount: total,
            status: "PENDING",        // Trạng thái mặc định 
            items: items.map((item: any) => ({
                dish_id: Number(item.id), // Bắt buộc là số, không được là "s1"
                quantity: item.quantity,
                price_at_order: item.price // Đúng tên trường trong Swagger
            }))
        };

        try {
            const response = await axios.post('http://localhost:3000/orders', orderPayload, {
                headers: { Authorization: `Bearer ${token}` } // Gửi kèm token để tránh lỗi 401
            });
            ...
            // Log chi tiết lỗi 400 từ Backend trả về
            console.log("Chi tiết lỗi 400:", error.response?.data);
            alert(error.response?.data?.message || "Lỗi dữ liệu, Dino kiểm tra Console nhé!");
        } finally {
